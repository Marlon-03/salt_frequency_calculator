'use client';
import { useState, useEffect, useCallback } from 'react';
import { UnitToggle } from './UnitToggle';
import {
  calcSaltToAdd,
  validateForwardInputs,
  formatNumber,
  type ForwardResult,
  type ValidationError,
} from '../lib/formulas';

const VOLUME_OPTS   = [{ value: 'gallons' as const, label: 'gal' }, { value: 'liters' as const, label: 'L' }];
const SALINITY_OPTS = [{ value: '%' as const, label: '%' }, { value: 'ppt' as const, label: 'ppt' }];
const SALT_OPTS     = [{ value: 'lbs' as const, label: 'lbs' }, { value: 'kg' as const, label: 'kg' }];

interface Fields { volume: string; currentSalinity: string; desiredSalinity: string; }

export function ForwardCalculator() {
  const [fields, setFields]             = useState<Fields>({ volume: '', currentSalinity: '', desiredSalinity: '' });
  const [volumeUnit, setVolumeUnit]     = useState<'gallons' | 'liters'>('gallons');
  const [salinityUnit, setSalinityUnit] = useState<'%' | 'ppt'>('%');
  const [saltUnit, setSaltUnit]         = useState<'lbs' | 'kg'>('lbs');
  const [result, setResult]             = useState<ForwardResult | null>(null);
  const [errors, setErrors]             = useState<ValidationError[]>([]);
  const [copied, setCopied]             = useState(false);
  const [animKey, setAnimKey]           = useState(0);

  const compute = useCallback(() => {
    const volume          = parseFloat(fields.volume);
    const currentSalinity = parseFloat(fields.currentSalinity);
    const desiredSalinity = parseFloat(fields.desiredSalinity);

    const errs = validateForwardInputs({
      volume:          isNaN(volume)          ? undefined : volume,
      currentSalinity: isNaN(currentSalinity) ? undefined : currentSalinity,
      desiredSalinity: isNaN(desiredSalinity) ? undefined : desiredSalinity,
      salinityUnit,
    });
    setErrors(errs);

    const hasError  = errs.some((e) => e.severity === 'error');
    const allFilled = !isNaN(volume) && !isNaN(currentSalinity) && !isNaN(desiredSalinity);

    if (!allFilled || hasError) { setResult(null); return; }

    const r = calcSaltToAdd({ volume, volumeUnit, currentSalinity, desiredSalinity, salinityUnit });
    setResult(r);
    setAnimKey((k) => k + 1);
  }, [fields, volumeUnit, salinityUnit]);

  useEffect(() => { compute(); }, [compute]);

  const set = (k: keyof Fields) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setFields((f) => ({ ...f, [k]: e.target.value }));

  const fieldErr    = (f: string) => errors.find((e) => e.field === f);
  const inputBorder = (f: string) => {
    const e = fieldErr(f);
    if (!e) return 'border-(--color-brand-border) focus-within:border-(--color-brand-red)';
    return e.severity === 'error'
      ? 'border-(--color-brand-danger)'
      : 'border-(--color-brand-warning)';
  };

  const handleCopy = async () => {
    if (!result) return;
    const val = saltUnit === 'lbs' ? result.saltLbs : result.saltKg;
    await navigator.clipboard.writeText(
      `Salt to add: ${formatNumber(val)} ${saltUnit}\n= ${formatNumber(result.saltLbs)} lbs / ${formatNumber(result.saltKg)} kg\nPond: ${formatNumber(result.volumeGallons)} gal / ${formatNumber(result.volumeLiters)} L`
    );
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const primaryVal   = result ? (saltUnit === 'lbs' ? result.saltLbs : result.saltKg) : null;
  const noSaltNeeded = result !== null && result.deltaPpt <= 0;

  return (
    <div className="contents">
      <div className="bg-(--color-brand-surface) border border-(--color-brand-border) rounded-2xl p-6 flex flex-col gap-5">
        <p className="text-xs font-semibold uppercase tracking-widest text-(--color-brand-text-muted)">Inputs</p>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="fwd-vol" className="text-sm font-medium text-(--color-brand-text-muted)">
            Pond Volume
          </label>
          <div className={`flex rounded-xl border overflow-hidden bg-(--color-brand-panel) transition-colors ${inputBorder('volume')}`}>
            <input
              id="fwd-vol"
              type="number" min="0" step="any"
              placeholder="e.g. 1000"
              value={fields.volume}
              onChange={set('volume')}
              className="flex-1 min-w-0 bg-transparent px-4 py-3 text-(--color-brand-text) font-mono text-sm outline-none placeholder:text-(--color-brand-muted)"
            />
            <UnitToggle options={VOLUME_OPTS} value={volumeUnit} onChange={setVolumeUnit} />
          </div>
          <FieldMsg err={fieldErr('volume')} />
        </div>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="fwd-cur" className="text-sm font-medium text-(--color-brand-text-muted)">
            Current Salinity
          </label>
          <div className={`flex rounded-xl border overflow-hidden bg-(--color-brand-panel) transition-colors ${inputBorder('currentSalinity')}`}>
            <input
              id="fwd-cur"
              type="number" min="0" step="any"
              placeholder="e.g. 0"
              value={fields.currentSalinity}
              onChange={set('currentSalinity')}
              className="flex-1 min-w-0 bg-transparent px-4 py-3 text-(--color-brand-text) font-mono text-sm outline-none placeholder:text-(--color-brand-muted)"
            />
            <UnitToggle options={SALINITY_OPTS} value={salinityUnit} onChange={setSalinityUnit} />
          </div>
          <FieldMsg err={fieldErr('currentSalinity')} />
        </div>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="fwd-des" className="text-sm font-medium text-(--color-brand-text-muted)">
            Desired Salinity
          </label>
          <div className={`flex rounded-xl border overflow-hidden bg-(--color-brand-panel) transition-colors ${inputBorder('desiredSalinity')}`}>
            <input
              id="fwd-des"
              type="number" min="0" step="any"
              placeholder="e.g. 0.3"
              value={fields.desiredSalinity}
              onChange={set('desiredSalinity')}
              className="flex-1 min-w-0 bg-transparent px-4 py-3 text-(--color-brand-text) font-mono text-sm outline-none placeholder:text-(--color-brand-muted)"
            />
            <UnitToggle options={SALINITY_OPTS} value={salinityUnit} onChange={setSalinityUnit} />
          </div>
          <FieldMsg err={fieldErr('desiredSalinity')} />
        </div>

        <div className="flex flex-col gap-1.5">
          <p className="text-sm font-medium text-(--color-brand-text-muted)">Show result in</p>
          <div className="flex gap-2">
            {SALT_OPTS.map((opt) => (
              <button
                key={opt.value} type="button"
                onClick={() => setSaltUnit(opt.value)}
                className={[
                  'flex-1 py-2.5 rounded-xl text-sm font-medium border transition-all duration-150 cursor-pointer',
                  saltUnit === opt.value
                    ? 'bg-(--color-brand-red) border-(--color-brand-red) text-white'
                    : 'border-(--color-brand-border) text-(--color-brand-text-muted) hover:border-(--color-brand-red) hover:text-(--color-brand-text)',
                ].join(' ')}
              >
                {opt.value === 'lbs' ? 'Pounds (lbs)' : 'Kilograms (kg)'}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* ── Result Card ── */}
      <div className="bg-linear-to-br from-(--color-brand-surface) to-(--color-brand-black) border border-(--color-brand-border) rounded-2xl p-6 flex flex-col gap-4 relative overflow-hidden">
        <div className="absolute -bottom-10 -right-10 w-40 h-40 rounded-full bg-brand-red-5 blur-2xl pointer-events-none" />
        <p className="text-xs font-semibold uppercase tracking-widest text-(--color-brand-text-muted)">Salt to Add</p>

        {primaryVal !== null && !noSaltNeeded ? (
          <div key={animKey} className="flex flex-col gap-4 animate-ripple-in">
            <div className="flex items-baseline gap-2">
              <span className="font-mono text-5xl font-medium text-(--color-brand-red) leading-none">
                {formatNumber(primaryVal)}
              </span>
              <span className="text-lg text-(--color-brand-text-muted)">{saltUnit}</span>
            </div>

            <div className="flex flex-col gap-2 pt-3 border-t border-(--color-brand-border)">
              <Row label="Also in"        value={saltUnit === 'lbs' ? `${formatNumber(result!.saltKg)} kg` : `${formatNumber(result!.saltLbs)} lbs`} />
              <Row label="Pond volume"    value={`${formatNumber(result!.volumeGallons)} gal / ${formatNumber(result!.volumeLiters)} L`} />
              <Row label="Salinity" value={`+${formatNumber(result!.deltaPpt, 3)} ppt`} />
            </div>

            <button
              type="button" onClick={handleCopy}
              className={[
                'self-start mt-1 px-4 py-2 rounded-lg text-xs border transition-all duration-200 cursor-pointer',
                copied
                  ? 'border-(--color-brand-success) text-(--color-brand-success)'
                  : 'border-(--color-brand-border) text-(--color-brand-text-muted) hover:border-(--color-brand-red) hover:text-(--color-brand-red)',
              ].join(' ')}
            >
              {copied ? '✓ Copied!' : '⧉ Copy result'}
            </button>
          </div>

        ) : noSaltNeeded ? (
          <div className="flex flex-col items-center justify-center flex-1 py-10 gap-2 text-center animate-fade-in">
            <span className="text-4xl">✓</span>
            <p className="font-semibold text-(--color-brand-text)">No salt needed</p>
            <p className="text-sm text-(--color-brand-text-muted)">Your pond is already at or above the desired salinity.</p>
          </div>

        ) : (
          <div className="flex flex-col items-center justify-center flex-1 py-10 gap-3 text-(--color-brand-text-muted) text-center">
            <svg className="opacity-30" width="44" height="44" viewBox="0 0 44 44" fill="none">
              <circle cx="22" cy="22" r="20" stroke="currentColor" strokeWidth="1.5" />
              <path d="M22 13v11" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
              <circle cx="22" cy="30" r="1.75" fill="currentColor" />
            </svg>
            <p className="text-sm">Enter your pond details to calculate</p>
          </div>
        )}
      </div>
    </div>
  );
}

function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-4">
      <span className="text-xs font-semibold uppercase tracking-wider text-(--color-brand-text-muted) opacity-60">{label}</span>
      <span className="text-sm text-(--color-brand-text) font-mono">{value}</span>
    </div>
  );
}

function FieldMsg({ err }: { err?: ValidationError }) {
  if (!err) return null;
  const isError = err.severity === 'error';
  return (
    <p className={`text-xs flex items-start gap-1.5 leading-relaxed ${isError ? 'text-(--color-brand-danger)' : 'text-(--color-brand-warning)'}`}>
      <span className="mt-0.5">{isError ? '✖' : '⚠'}</span>
      {err.message}
    </p>
  );
}
